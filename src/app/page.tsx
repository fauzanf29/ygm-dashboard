...
export default function Page() {
    ...
    try {
      ...
      if (res.ok) {
        if (action === 'checkout') { 
            ...
            setTodaySales(0); 
            setTodayItems([]); 
            localStorage.removeItem('ygm_checkInTime'); 
            localStorage.removeItem('ygm_todaySales'); 
            localStorage.removeItem('ygm_todayItems'); 
        } 
        else { 
            setIsCheckedIn(true); 
            localStorage.setItem('ygm_checkInTime', Date.now().toString()); 
        }
        fetchMgmtStats();
      }
    } catch (e) {}
  ...
  const submitSales = async () => {
    ...
    try {
      ...
      localStorage.setItem('ygm_todaySales', newTotalSales.toString());
      setTodayItems(prev => { 
          ...
          localStorage.setItem('ygm_todayItems', JSON.stringify(updatedItems)); 
          return updatedItems; 
      });
  ...
  // Effects
  useEffect(() => { let interval: NodeJS.Timeout; if (isCheckedIn) { interval = setInterval(() => { const saved = localStorage.getItem('ygm_checkInTime'); if (saved) setSeconds(Math.floor((Date.now() - parseInt(saved)) / 1000)); else setSeconds(prev => prev + 1); }, 1000); } return () => clearInterval(interval); }, [isCheckedIn]);
  useEffect(() => { const savedCheckIn = localStorage.getItem('ygm_checkInTime'); if (savedCheckIn) { setIsCheckedIn(true); setSeconds(Math.floor((Date.now() - parseInt(savedCheckIn)) / 1000)); } const savedSales = localStorage.getItem('ygm_todaySales'); if (savedSales) setTodaySales(parseInt(savedSales)); const savedItems = localStorage.getItem('ygm_todayItems'); if (savedItems) setTodayItems(JSON.parse(savedItems)); }, []);
